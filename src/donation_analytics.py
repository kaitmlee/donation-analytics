...
def scan(donors, percentile):
    ...
    for index, row in donors.iterrows():
        #already in list
        if (row["NAME"], row["ZIP_CODE"]) in repeat_donors:
            #if year2<year1 then update value in dict/drop row from df.
            if(row["TRANSACTION_DT"]<repeat_donors[row["NAME"], row["ZIP_CODE"]]):
                repeat_donors[row["NAME"], row["ZIP_CODE"]] = row["TRANSACTION_DT"]
...
def findPercentile(amounts, percentile):
    #find the rank
...
def output(donors, index, percentile):
    ...
    perc = findPercentile(amounts, percentile)
    amt = sum(amounts)
    cnt = len(amounts)
